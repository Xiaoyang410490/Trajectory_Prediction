...
def train_step(input, target, dec_in):
    ...
    with tf.GradientTape() as tape:
        ...
        enc_hidden_1 = encoder.initialize_hidden_state()
        enc_cell_1 = encoder.initialize_cell_state()

        enc_output, enc_hidden, enc_cell, enc_hidden_1, enc_cell_1 = encoder(input, enc_hidden, enc_cell, enc_hidden_1, enc_cell_1,'train')
        ...
        for t in range(8):
            ...
            predictions, dec_hidden, dec_cell,dec_hidden_1,dec_cell_1 = decoder(dec_input, enc_hidden, dec_hidden, dec_cell, dec_hidden_1,dec_cell_1, enc_output, 'train')
...
def metrics():
    ...
    for (batch, (inp, targ)) in enumerate(dataset_valid.take(Nummer)):
        ...
        enc_out, enc_hidden, enc_cell,enc_hidden_1, enc_cell_1 = encoder(input_encoder, enc_hidden, enc_cell, enc_hidden_1, enc_cell_1,'test')
        ...
        for ind in range(8):
            ...
            predictions, dec_hidden, dec_cell,dec_hidden_1,dec_cell_1 = decoder(dec_input, enc_hidden, dec_hidden, dec_cell,dec_hidden_1,dec_cell_1, enc_out, 'test')
            predictions = tf.squeeze(predictions, 1)
