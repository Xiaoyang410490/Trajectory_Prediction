...
def ang_computation(pos_x,pos_y):
    ...
    #num,num_1 are start and end point for each curve
    num = 0
    dist = 0
    while (dist < dis):
        num = num + 1
...
def target_input_get(fp1):
    ...
    for i in range(249):
        ...
        vel,vel_in = vel_computation(pos_x, pos_y, num, num_1)

        # for every curve we take 150 position data
        if len(vel)>150:

           for m in range(150):
                info.append([angle[m],vel[m]])
           target_data.append(info)

           krummung = findlane_index(pos_x,pos_y)
           ...
           for t in range(num,num+150):
               ...
               x_norm = (pos_x[t] - x_init)/30 + 0.05
               y_norm = (pos_y[t] - y_init)/30 + 0.05
               pfai_norm = pfai[t-num]/math.pi + 0.03
               info2.append([x_norm,y_norm,pfai_norm,beta[t-num],krummung[t],vel_in[t-num]])

           input_data.append(info2)

    return target_data,input_data

def train_valid_get3():

    target_data, input_data= target_input_get('allAmsterdamerRing.pickle')

    #traing_set:validation_set = 4:1
    train_split = math.floor(128)
