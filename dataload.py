...
def sample_data(rand,ang,vel,acc,target_data,
    ...
    x_init = pos_x[startpoint]
    y_init = pos_y[endpoint]
    ...
    for t in range(startpoint, endpoint):
        ...
        if m % 10 == 0:
            ...
            info3.append([lis1[t], lis2[t], lis3[t]])
...
def target_input_get(fp1):
    ...
    for i in range(len(data)):
        ...
        kru5, ori5, rul5, fai5, \
        kru10, ori10, rul10, fai10, \
        kru15, ori15, rul15, fai15 = findlane_index(pos_x,pos_y,kind)
...
def train_valid_get():
    ...
    right = [0, 9, 10, 11, 12, 16, 32, 33, 34, 36, 37, 46, 47, 48, 49, 50, 54, 55, 73, 74, 76, 77, 78, 79, 80, 81, 82,
             83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103,
             106, 110, 111, 112, 113, 114, 115, 118, 119, 120, 121, 122, 123, 124, 126, 131, 132, 133, 134, 135, 137,
             138, 139, 140, 141, 142, 143, 146, 148, 149, 163, 164, 165, 166, 167, 171, 182,
             185, 194, 195, 196, 197, 198, 206, 207, 208, 209, 210, 212, 215, 219, 221, 222, 223, 224, 225, 226, 227,
             228, 229, 230, 231, 235, 251, 252, 266, 267, 268]

    left = [0, 1, 2, 3, 4, 5, 6, 8, 14, 15, 16, 17, 18, 19, 20, 21, 22, 32, 33, 34, 35, 52, 58, 59, 60, 61, 62, 63, 64,
            65, 66, 67, 68, 69, 70, 72, 73, 74, 75, 76, 78, 79, 80, 81, 82, 84, 85, 86, 106, 114, 115,
            134, 136, 141, 142, 143, 144, 145, 146, 154, 155, 156, 157, 160, 161, 162, 163, 164, 165, 166, 167, 168,
            171, 172, 173, 174, 175, 176, 177, 181, 190, 191, 193, 194, 195, 196, 197, 198,
            202, 217, 218, 219, 220, 221, 222, 223, 224, 225, 226, 227, 228, 229, 233, 234, 235, 236, 237, 238, 240,
            241, 242, 243, 244, 245, 246, 252, 253, 254, 274, 275, 278, 279, 280]
...
def check_data():
    ...
    for i in range(len(target_data_s)):
        ...
        for t in range(len(training_d)):
            ...
            pox.append(input_per_time[0] * 10)
            poy.append(input_per_time[1] * 10)

            kru.append(input_per_time[7] * 0.1)
            kru5.append(input_per_time[10] * 0.1)
            kru10.append(input_per_time[14] * 0.1)
            kru15.append(input_per_time[18] * 0.1)

            ori.append(input_per_time[9])
            ori5.append(input_per_time[12])
            ori10.append(input_per_time[16])
            ori15.append(input_per_time[20])

            gam.append(input_per_time[22])
            gam5.append(input_per_time[23])
            gam10.append(input_per_time[24])
            gam15.append(input_per_time[25])

            psi.append(input_per_time[4])
            beta.append(input_per_time[5])
            velo.append(input_per_time[6])
        ...
        plt.title('angle difference between psi and orientation')
        plt.plot(gam)
        plt.plot(gam5)
        plt.plot(gam10)
        plt.plot(gam15)
