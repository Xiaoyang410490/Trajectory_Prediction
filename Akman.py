...
def akman(acc,angle,beta_init,x_init,y_init,pfai_init,vel_init):
    beta = []
...
def acc_computation(pos_x,pos_y,dt):
     for i in range(1,len(pos_x)-1):
        dx1 = pos_x[i] - pos_x[i-1]
        dy1 = pos_y[i] - pos_y[i-1]
...
if __name__ == '__main__':
      ...
      cd = load_data_files('allAmsterdamerRing.pickle',1)
      ...
      angle = cal2(pos_x,pos_y,angle)

      acc = acc_computation(pos_x,pos_y,dt)
