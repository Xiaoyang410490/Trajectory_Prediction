...
def findlane_index(pos_x,pos_y):
    ...
    lane_bevor = find_lane1(pos_x[0], pos_y[0], pos_x[1], pos_y[1],map)
    list1 = []
    list1.append(lane_bevor)
    for i in range(1, len(pos_x)):
        lane = find_lane2(pos_x[i - 1], pos_y[i - 1], pos_x[i], pos_y[i], lane_bevor,map,graph)
        list1.append(lane)
        lane_bevor = lane

    lane_bevor_re = find_lane1(pos_xre[0],pos_yre[0], pos_xre[1], pos_yre[1],map)
    list2 = []
    list2.append(lane_bevor)
    for i in range(1, len(pos_xre)):
        lane = find_lane2(pos_xre[i - 1], pos_yre[i - 1], pos_xre[i], pos_yre[i], lane_bevor_re,map,graph)
        list2.append(lane)
        lane_bevor_re = lane
    list2 = list2[::-1]
    ...
    for i in range(len(index_list1)):
        lanelet = map.laneletLayer[index_list1[i]]
        kru.append(Krummung_rechnen(lanelet))

    return kru

if __name__ == '__main__':

    cd = load_data_files('allAmsterdamerRingV2.pickle',4)
    x = np.array(cd['pos_x'])
    y = np.array(cd['pos_y'])
    list = findlane_index(x,y)
    print(list)
