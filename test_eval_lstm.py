...
def train_step(input,target,dec_in):
    ...
    with tf.GradientTape() as tape:
        ...
        for t in range(15):
            ...
            predictions, dec_hidden, dec_cell = decoder(dec_input, dec_hidden, dec_cell, enc_output,'train')
    ...
    for i in range(Nummer):
        ...
        for t in range(30, 135):
            ...
            for ind in range(15):
                ...
                predictions, dec_hidden, dec_cell = decoder(dec_input, dec_hidden, dec_cell, enc_out,'test')
...
decoder = Decoder_lstm(out_size, units, Batch_size)
...
def show():
    ...
    steps_per_valid = len(x_valid)//Batch_size
    print(steps_per_valid)
    for k in range(1):
         input_valid, target_valid = next(iter(dataset_valid))

    input_valid_time = tf.transpose(input_valid, [1, 0, 2])
    ...
    input_encoder = input_valid_time[t - 30:t]
    input_encoder = tf.transpose(input_encoder, [1, 0, 2])
    input_decoder = input_valid_time[t:t + 15]
    input_decoder = tf.transpose(input_decoder, [1, 0, 2])
