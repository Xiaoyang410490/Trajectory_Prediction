...
def train_step(input,target,dec_in):
    ...
    with tf.GradientTape() as tape:
        ...
        hidden = grulayer.initialize_hidden_state()

        output, hidden= grulayer(input, hidden,'train')

        for t in range(8):

            dec_input = tf.expand_dims(dec_in[:, t], 1)

            predictions, hidden = grulayer(dec_input, hidden ,'train')
    ...
    #optimization_step
    variables = grulayer.trainable_variables
...
grulayer = gru_model(units,Batch_size)

#set the optimizer,commonly we use the adam optimizer
optimizer = tf.keras.optimizers.Adam()

#set the checkpoint
checkpoint_dir = './model_gru_1_checkpoints'
...
def metrics():
    ...
    for (batch, (inp, targ)) in enumerate(dataset_valid.take(Nummer)):
        ...
        enc_out, hidden = grulayer(input_encoder, hidden , 'test')
        ...
        for ind in range(8):
            ...
            predictions, hidden = grulayer(dec_input, hidden, 'test')
            predictions = tf.squeeze(predictions, 1)
