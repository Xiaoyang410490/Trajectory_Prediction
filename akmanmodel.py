...
def input_data(target_index,num_index,file_path,velocity,winkel):
    ...
    for i in range(len(target_index)):
        ...
        pfai = akman(file_path,vel,angle,index,num)

        for i in range(num,num+150):
            info.append([pos_x[i],pos_y[i],pfai[i-num],krummung[i]])
...
if __name__ == '__main__':
    target_data,target_index,num_index,velocity,winkel = target_data('allAmsterdamerRingV2.pickle')
    input_data = input_data(target_index=target_index,num_index=num_index,file_path='allAmsterdamerRingV2.pickle',velocity=velocity,winkel=winkel)
