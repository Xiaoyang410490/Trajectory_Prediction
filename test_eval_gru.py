...
def train_step(input,target,dec_in):
    ...
    with tf.GradientTape() as tape:
        ...
        for t in range(7):
            ...
            predictions, dec_hidden = decoder(dec_input, enc_hidden, dec_hidden, enc_output, 'train')
...
decoder = Decoder_Attention(out_size, units, Batch_size)
...
def metrics():
    ...
    for (batch, (inp, targ)) in enumerate(dataset_valid.take(Nummer)):
        ...
        enc_out, enc_hidden = encoder(input_encoder, hidden_init)
        ...
        for ind in range(1, 7):
            ...
            predictions, hidden = decoder(dec_input, enc_hidden,hidden, enc_out, 'test')
            predictions = tf.squeeze(predictions, 1)
...
def visualization():
    ...
    # seq2seq model
    hidden_init = encoder.initialize_hidden_state()
    enc_out, enc_hidden = encoder(input_encoder, hidden_init, 'test')
    hidden = enc_hidden
