...
def train_step(input,target,dec_in):
    ...
    with tf.GradientTape() as tape:
        ...
        for t in range(15):
            ...
            predictions, dec_hidden = decoder(dec_input, dec_hidden, enc_output, 'train')
...
def show_real():
    ...
    input_valid, target_valid = next(iter(data_valid))
    ...
    for i in range(16):
        ...
        pfai_ = kurve_initial[2]
        beta_ = kurve_initial[3]
        vel_ = kurve_initial[7]

        pre_x.append(x_)
        pre_y.append(y_)
        pre_pfai.append(pfai_)
        pre_beta.append(beta_)
        pre_velo.append(vel_)
