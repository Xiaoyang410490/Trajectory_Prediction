...
def train_step(input,target,dec_in):
    ...
    with tf.GradientTape() as tape:
        ...
        enc_hidden = enc_dec.initialize_hidden_state()
        enc_cell = enc_dec.initialize_cell_state()
        enc_hidden_1 = enc_dec.initialize_hidden_state_1()
        enc_cell_1 = enc_dec.initialize_cell_state_1()

        enc_output, enc_hidden, enc_cell, enc_hidden_1, enc_cell_1= enc_dec(input, enc_hidden, enc_cell, enc_hidden_1, enc_cell_1, mode='encoder', mode_1 ='train')

        dec_hidden = enc_hidden
        dec_cell = enc_cell
        dec_hidden_1 = enc_hidden_1
        dec_cell_1 = enc_cell_1
        ...
        for t in range(15):
            ...
            predictions, dec_hidden, dec_cell, dec_hidden_1, dec_cell_1 = enc_dec(dec_input, dec_hidden, dec_cell, dec_hidden_1, dec_cell_1, mode ='decoder', mode_1 ='train')
...
def test_set():
    ...
    for i in range(Nummer):
        ...
        for t in range(30, 135):
            ...
            hidden = enc_dec.initialize_hidden_state()
            cell = enc_dec.initialize_cell_state()
            hidden_1 = enc_dec.initialize_hidden_state_1()
            cell_1 = enc_dec.initialize_cell_state_1()

            enc_out, enc_hidden, enc_cell, enc_hidden_1, enc_cell_1  = enc_dec(input_encoder, hidden, cell, hidden_1, cell_1, mode='encoder',mode_1='test')
            ...
            for ind in range(15):
                ...
                predictions, dec_hidden, dec_cell, hidden_1, cell_1 = enc_dec(dec_input, dec_hidden, dec_cell, dec_hidden_1, dec_cell_1,mode='decoder',mode_1='test' )
...
enc_dec = Encoder_decoder(units,Batch_size,out_size)

#set the optimizer,commonly we use the adam optimizer
optimizer = tf.keras.optimizers.Adam()

#set the checkpoint
checkpoint_dir = './one_cell_checkpoints'
...
def show():
    ...
    enc_out, enc_hidden, enc_cell, enc_hidden_1, enc_cell_1  = enc_dec(input_encoder, hidden, cell, hidden_1, cell_1, mode='encoder',mode_1='train')
    ...
    for ind in range(1, 15):
        ...
        predictions, dec_hidden, dec_cell, hidden_1, cell_1 = enc_dec(dec_input, dec_hidden, dec_cell, hidden_1, cell_1, 'decoder','test')
        predictions = tf.squeeze(predictions, 1)
