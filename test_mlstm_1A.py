...
def train_step(input,target,dec_in):
    ...
    with tf.GradientTape() as tape:
        ...
        output, hidden, cell, hidden_1, cell_1 = lstmlayer(input, hidden, cell, hidden_1, cell_1 ,'train',hidden,'encoder')
        enc_hidden = hidden
        ...
        for t in range(8):
            ...
            predictions, hidden, cell, hidden_1, cell_1 = lstmlayer(dec_input, hidden, cell, hidden_1, cell_1,'train',enc_hidden,'decoder')
...
def metrics():
    ...
    for (batch, (inp, targ)) in enumerate(dataset_valid.take(Nummer)):
        ...
        enc_out, hidden, cell, hidden_1, cell_1 = lstmlayer(input_encoder, hidden, cell, hidden_1, cell_1,'test',hidden,'encoder')
        enc_hidden = hidden
        pre_x = []
        pre_y = []
        pre_pfai = []
        ...
        for ind in range(8):
            ...
            predictions, hidden, cell, hidden_1, cell_1 = lstmlayer(dec_input, hidden, cell, hidden_1, cell_1, 'test',enc_hidden,'decoder')
            predictions = tf.squeeze(predictions, 1)
