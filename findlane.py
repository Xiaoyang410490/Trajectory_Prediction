...
def future_parameter(future_len,mapgeometry,posx,posy,laneid,laneid_2,laneid_3,lane,lane2,lane3):
    ...
    if length_along_center_1 < 0:
        ...
        if initial_id in turn_list:
            kappa_f = mapgeometry.calculateSplineCurvature(x_spl_3, y_spl_3, initial_length)
        else:
            kappa_f = 0.0

        ori_f = mapgeometry.calculateSplineOrientation(x_spl_3, y_spl_3, initial_length)
        rul_f = rule_info(laneid_3)
...
def findlane_index(pos_x,pos_y,kind,num):
    ...
    for i in range(len(list1)):
        ...
        if k < (len(list0) - 2):
            ...
            final_id = list0[k]
            next_lanelet_2 = graph.following(map.laneletLayer[final_id])
            if len(next_lanelet_2) != 0:
                next_lane_2 = next_lanelet_2[0]
                final_id_2=  next_lane_2.id
            else:
                final_id_2 = final_id

            next_lanelet_3 = graph.following(map.laneletLayer[final_id_2])
            if len(next_lanelet_3) != 0:
                next_lane_3 = next_lanelet_3[0]
                id_3 = next_lane_3.id
            else:
                id_3 = final_id_2
