...
class Decoder_Attention(tf.keras.Model):
    ...
    def call(self, x, enc_hidden,hidden, enc_output, mode):

        # enc_output shape == (batch_size, max_length, hidden_size)
        context_vector, attention_weights = self.attention(enc_hidden, enc_output)
...
class Decoder(tf.keras.Model):
    ...
    def call(self, x, enc_hidden, hidden, enc_output, mode):
        ...
        # passing the concatenated vector to the GRU
        output, state = self.gru(input_x,initial_state=hidden)
